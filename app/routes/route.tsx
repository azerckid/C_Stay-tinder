...
// Mock 데이터: 이동 시간 및 거리 계산 (실제로는 API에서 가져올 예정)
function calculateRouteInfo(destinations: Destination[]) {
  // 간단한 Mock 계산
  const totalPlaces = destinations.length;
  const estimatedTimePerPlace = 2; // 시간
  const estimatedTravelTime = (totalPlaces - 1) * 0.5; // 이동 시간 (시간)
  const totalTime = totalPlaces * estimatedTimePerPlace + estimatedTravelTime;
  const totalDistance = totalPlaces * 4; // km
...
// Mock 이동 수단 타입
type TransportType = "car" | "walk";

function getTransportInfo(index: number): { type: TransportType; time: number; label: string } {
  // 첫 번째 이동은 차량, 나머지는 도보로 가정
  if (index === 0) {
    return { type: "car", time: 15, label: "차량 15분 이동" };
  }
...
function getCategoryColor(tag: string): string {
  if (tag.includes("바다") || tag.includes("해변")) {
    return "bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400";
  }
  if (tag.includes("액티비티") || tag.includes("활기")) {
    return "bg-purple-100 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400";
  }
  if (tag.includes("관광") || tag.includes("명소")) {
...
export default function RoutePage() {
  ...
  const routeInfo = calculateRouteInfo(selectedDestinations);
  const isActive = (path: string) => location.pathname === path;

  // 선택된 여행지가 없으면 홈으로 리다이렉트
  if (selectedDestinations.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-background-dark text-white px-6">
        <p className="text-xl font-bold mb-4">선택된 여행지가 없습니다</p>
  ...
  return (
    <div className="relative flex h-full min-h-screen w-full flex-col overflow-hidden max-w-md mx-auto bg-background-dark text-white">
        ...
        <div className="w-full h-full bg-gray-800 relative overflow-hidden">
          ...
          {selectedDestinations.map((dest, index) => {
            ...
            return (
              ...
              >
                <div
                  className={`text-white text-xs font-bold px-2 py-1 rounded-lg mb-1 shadow-lg whitespace-nowrap ${
                    isFirst ? "bg-primary" : "bg-surface-dark border border-gray-700"
                  }`}
                >
                ...
                <div
                  className={`size-4 bg-white rounded-full shadow-lg ${
                    isFirst ? "border-4 border-primary" : "border-4 border-gray-500"
                  }`}
                />
              </div>
            );
            ...
            {selectedDestinations.map((dest, index) => {
              ...
              return (
                ...
                >
                  {/* Timeline Dot */}
                  <div
                    className={`absolute -left-[25px] top-0 size-5 rounded-full border-4 border-background-dark shadow-sm z-10 ${
                      isFirst ? "bg-primary" : "bg-gray-600"
                    }`}
                  />
      ...
      <div className="p-4 bg-background-dark border-t border-gray-800">
        <Button
          className="w-full flex items-center justify-center gap-2 bg-primary hover:bg-primary/90 text-white font-bold py-4 px-6 rounded-xl shadow-lg shadow-primary/20 transition-all active:scale-[0.98]"
        >
